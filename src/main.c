	...
	while(1) {
		...
		*/
		...
		case FILL_BUF:
			...
			break;
		case QUIT:
			if(is_data_saved == DATA_NO_HAS_BEEN_SAVED && buffer_allocated_memory_flag == BUFFER_HAS_MEMORY) {
...
/* new commands:
	bf [filename] - fill buffer from file ----------------- X

	привести все сообщения об ошибках к одному формату
*/
